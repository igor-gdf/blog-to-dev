<div class="overflow-a row g-3" style="max-height: 83vh;">
    ...
    <?php if (!empty($currentUser) && ($post['Post']['user_id'] == $currentUser['id'] || $currentUser['role'] === 'admin')): ?>
        <div class="d-flex gap-2">
            <?= $this->Html->link(__('Editar'), ['action' => 'edit', $post['Post']['id']], ['class' => 'btn btn-warning']) ?>

            <?= $this->Form->postLink(
                __('Deletar'),
                ['action' => 'delete', $post['Post']['id']],
                ['class' => 'btn btn-danger', 'confirm' => __('Tem certeza que deseja deletar este post?', $post['Post']['id'])]
            ) ?>
            ...
            <?php if ($post['Post']['status'] === 'draft'): ?>
                <form action="<?= $this->Html->url(array('controller' => 'posts', 'action' => 'edit', $post['Post']['id'])) ?>"
                    ...
                    <button type="submit" class="btn btn-success">Publicar rascunho</button>
                </form>
            <?php else: ?>
                <form action="<?= $this->Html->url(array('controller' => 'posts', 'action' => 'edit', $post['Post']['id'])) ?>"
                    method="post">
