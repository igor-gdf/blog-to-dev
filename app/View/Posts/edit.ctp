...
<form action="<?= $this->Html->url(array('controller' => 'posts', 'action' => 'edit', $post['Post']['id'])) ?>"
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="title" name="data[Post][title]"
            value="<?= h($post['Post']['title']) ?>" required>
    </div>
    <div class="mb-3">
        <label for="content" class="form-label">Conteúdo</label>
        <textarea class="form-control" id="content" name="data[Post][content]" rows="8"
            required><?= h($post['Post']['content']) ?></textarea>
    </div>
    ...
    <div class="d-flex gap-2">
        ...
        <?= $this->Html->link('Cancelar', ['action' => 'view', $post['Post']['id']], ['class' => 'btn btn-secondary']) ?>
    </div>
</form>